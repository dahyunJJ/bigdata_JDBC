...
public class ParkingServiceImpl implements ParkingService{
	...
	public void outParking() {
		...
		// carNum 조회 
		HashMap<String, Object> parkingMap = parkingDAO.selectParkingInfo(location);		
		
		// parkingMap이 null인지 확인
    if (parkingMap.get("parkingCarNumber") == null) {
    	
      System.out.println("해당 위치에 주차된 차량 정보가 없습니다.");
      return;
      
    }	else {   
    	
    	String carNum = parkingMap.get("parkingCarNumber").toString();	   	
		
			boolean flag = parkingDAO.deleteParking(location);			
			...
			if (flag) {
				...
				//출차이력 등록
				parkingDAO.insertParkingHistory(location, carNum, "O");
